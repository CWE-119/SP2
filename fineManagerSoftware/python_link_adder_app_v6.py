...
def add_many_image_links_file(image_links_path: str, new_links: list[str], allow_duplicates: bool) -> tuple[bool, str, str | None]:
    js = read_text(image_links_path)
    m = _IMAGE_LINKS_RE.search(js)
    if not m:
        return False, "Could not find: export const imageLinks = [ ... ];", None
    ...
    if not to_add:
        return False, f"Nothing to add. Skipped={skipped}, Invalid={invalid}", None
    ...
    backup_path = make_backup(image_links_path)
    write_text(image_links_path, new_js)
    return True, f"Added={added}, Skipped={skipped}, Invalid={invalid} (backup: {os.path.basename(backup_path)})", backup_path


def remove_many_image_links_file(image_links_path: str, links_to_remove: list[str]) -> tuple[bool, str, str | None]:
    js = read_text(image_links_path)
    m = _IMAGE_LINKS_RE.search(js)
    if not m:
        return False, "Could not find: export const imageLinks = [ ... ];", None
    ...
    to_remove = [l for l in cleaned if l in existing_set]
    missing = len([l for l in cleaned if l not in existing_set])

    if not to_remove:
        return False, f"Nothing removed. Missing={missing}", None

    new_list = [l for l in existing if l not in set(to_remove)]

    indent = "  "
    if new_list:
        body_out = "\n" + "\n".join([f'{indent}"{l}"' for l in new_list]) + "\n"
    else:
        body_out = "\n"
    ...
    return True, f"Removed={len(to_remove)}, Missing={missing} (backup: {os.path.basename(backup_path)})", backup_path
...
def add_many_project_media_file(project_js_path: str, links: list[str], typ: str, media_type: str, allow_duplicates: bool) -> tuple[bool, str, str | None]:
    js = read_text(project_js_path)
    m = _MEDIA_DATA_RE.search(js)
    if not m:
        return False, "Could not find: const mediaData = [ ... ];", None
    ...
    new_js = js[:m.start()] + prefix + body_out + suffix + js[m.end():]
    backup_path = make_backup(project_js_path)
    write_text(project_js_path, new_js)
    return True, f"Added={added}, Skipped={skipped}, Invalid={invalid} (backup: {os.path.basename(backup_path)})", backup_path


def remove_many_project_media_file(project_js_path: str, links_to_remove: list[str]) -> tuple[bool, str, str | None]:
    js = read_text(project_js_path)
    m = _MEDIA_DATA_RE.search(js)
    if not m:
        return False, "Could not find: const mediaData = [ ... ];", None
    ...
    to_remove_set = set([l for l in cleaned if l in existing_set])
    missing = len([l for l in cleaned if l not in existing_set])

    if not to_remove_set:
        return False, f"Nothing removed. Missing={missing}", None

    kept_items = [it for it in items if it["link"] not in to_remove_set]
    ...
    if kept_items:
        new_blocks = [build_media_object(it["link"], it["type"], it["mediaType"], indent=indent) for it in kept_items]
        body_out = "\n" + ",\n".join(new_blocks) + "\n"
    else:
        body_out = "\n"
    ...
    return True, f"Removed={len(to_remove_set)}, Missing={missing} (backup: {os.path.basename(backup_path)})", backup_path
...
class LinkManagerApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("JS Link Manager (Bulk Add + Preview + Delete + Undo + Search)")
        ...
        # search / filter
        self.image_filter_var = tk.StringVar(value="")
        self.project_filter_var = tk.StringVar(value="")
        self._image_all: list[str] = []
        self._project_all: list[str] = []

        # undo backups
        self._last_backup_image: str | None = None
        self._last_backup_project: str | None = None

        # track what was edited in this session
        self._touched_image = False
        self._touched_project = False
    ...
    def _build_ui(self):
        ...
        form_frame = ttk.LabelFrame(self, text="Bulk Add / Delete Links (one per line)")
        ...
        ttk.Radiobutton(
            mode_row, text="Featured Image → imageLinks.js",
            variable=self.mode, value="featured",
            command=self.update_mode_ui
        ).pack(side="left", padx=6)

        ttk.Radiobutton(
            mode_row, text="Project Media → project.js",
        ...
        ttk.Button(options_row, text="Delete Selected", command=self.delete_selected).pack(side="right", padx=6)
        ttk.Button(options_row, text="Delete From Textbox", command=self.delete_from_textbox).pack(side="right", padx=6)
        ttk.Button(options_row, text="Undo Delete", command=self.undo_last_delete).pack(side="right", padx=6)
        ...
        # delete backups before push (optional, default false)
        self.delete_backups_before_push_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(
            row1,
            text="Delete backup files before push",
            variable=self.delete_backups_before_push_var
        ).pack(side="left", padx=6)
        ...
        ttk.Entry(row1, textvariable=self.commit_msg_var, width=50).pack(side="left", padx=6, fill="x", expand=True)

        # choose which files to stage/push
        row_files = ttk.Frame(git_frame)
        row_files.pack(fill="x", padx=8, pady=(0, 6))

        ttk.Label(row_files, text="Files to push (staged):").pack(side="left", padx=6)

        self.push_image_var = tk.BooleanVar(value=False)
        self.push_project_var = tk.BooleanVar(value=False)

        self.push_image_cb = ttk.Checkbutton(row_files, text="imageLinks.js", variable=self.push_image_var)
        self.push_project_cb = ttk.Checkbutton(row_files, text="project.js", variable=self.push_project_var)

        self.push_image_cb.pack(side="left", padx=6)
        self.push_project_cb.pack(side="left", padx=6)

        ttk.Button(row_files, text="Use touched files", command=self._select_touched_files).pack(side="left", padx=10)
        ttk.Button(row_files, text="Select none", command=lambda: (self.push_image_var.set(False), self.push_project_var.set(False))).pack(side="left", padx=6)
        ...
        self.gh_workflow_var = tk.StringVar(value="")
        ...
        self.gh_ref_var = tk.StringVar(value="")
        ...
        img_filter_row = ttk.Frame(left)
        img_filter_row.pack(fill="x", pady=(4, 0))
        ttk.Entry(img_filter_row, textvariable=self.image_filter_var).pack(side="left", fill="x", expand=True)
        ttk.Button(img_filter_row, text="Clear", command=lambda: self.image_filter_var.set("")).pack(side="left", padx=6)
        self.image_filter_var.trace_add("write", lambda *_: self.apply_filters())

        self.image_list = tk.Listbox(left, height=18, selectmode="extended")
        ...
        prj_filter_row = ttk.Frame(mid)
        prj_filter_row.pack(fill="x", pady=(4, 0))
        ttk.Entry(prj_filter_row, textvariable=self.project_filter_var).pack(side="left", fill="x", expand=True)
        ttk.Button(prj_filter_row, text="Clear", command=lambda: self.project_filter_var.set("")).pack(side="left", padx=6)
        self.project_filter_var.trace_add("write", lambda *_: self.apply_filters())

        self.project_list = tk.Listbox(mid, height=18, selectmode="extended")
    ...
    # ----------------------------
    # touched -> selection helpers
    # ----------------------------

    def _mark_touched_image(self):
        self._touched_image = True
        self._select_touched_files()

    def _mark_touched_project(self):
        self._touched_project = True
        self._select_touched_files()

    def _select_touched_files(self):
        # default behavior: stage only what was edited in this session
        self.push_image_var.set(bool(self._touched_image))
        self.push_project_var.set(bool(self._touched_project))
    ...
    def _build_context_menus(self):
        self.img_menu = tk.Menu(self, tearoff=0)
        self.img_menu.add_command(label="Open link", command=lambda: self._ctx_open("image"))
        self.img_menu.add_command(label="Copy link", command=lambda: self._ctx_copy("image"))
        self.img_menu.add_separator()
        self.img_menu.add_command(label="Delete", command=lambda: self._ctx_delete("image"))
        self.img_menu.add_separator()
        self.img_menu.add_command(label="Undo last delete", command=self.undo_last_delete)

        self.prj_menu = tk.Menu(self, tearoff=0)
        self.prj_menu.add_command(label="Open link", command=lambda: self._ctx_open("project"))
        self.prj_menu.add_command(label="Copy link", command=lambda: self._ctx_copy("project"))
        self.prj_menu.add_separator()
        self.prj_menu.add_command(label="Delete", command=lambda: self._ctx_delete("project"))
        self.prj_menu.add_separator()
        self.prj_menu.add_command(label="Undo last delete", command=self.undo_last_delete)

        self.image_list.bind("<Button-3>", lambda e: self._show_ctx_menu(e, "image"))
        self.project_list.bind("<Button-3>", lambda e: self._show_ctx_menu(e, "project"))

    def _show_ctx_menu(self, event, kind: str):
        lb = self.image_list if kind == "image" else self.project_list
        ...
        if kind == "image":
            ...
            self.preview_url(url)
            self.prj_menu.tk_popup(event.x_root, event.y_root)

    def _ctx_get_selected_links(self, kind: str) -> list[str]:
        if kind == "image":
            sel = self.image_list.curselection()
            return [self.image_list.get(i).strip() for i in sel] if sel else []
        else:
            sel = self.project_list.curselection()
            out = []
            for i in sel:
                row = self.project_list.get(i)
                parts = row.split(" | ")
                url = parts[-1].strip() if parts else ""
                if url:
                    out.append(url)
    ...
    def _ctx_open(self, kind: str):
        links = self._ctx_get_selected_links(kind)
        if not links:
            return
        webbrowser.open(links[0])

    def _ctx_copy(self, kind: str):
        links = self._ctx_get_selected_links(kind)
        if not links:
            return
    ...
    def _ctx_delete(self, kind: str):
        links = self._ctx_get_selected_links(kind)
        if not links:
            return
        if not messagebox.askyesno("Delete", f"Delete {len(links)} selected link(s)?"):
            return
        ...
        if kind == "image":
            ...
            if not os.path.exists(path):
                ...
                return
            ok, msg, backup = remove_many_image_links_file(path, links)
            if ok:
                self._last_backup_image = backup
                self._mark_touched_image()
                messagebox.showinfo("Deleted", msg)
                self.refresh_previews()
                if self.auto_commit_var.get():
                    self.git_push()
            else:
                messagebox.showwarning("Not deleted", msg)
        else:
            path = self.project_js_path.get().strip()
            if not os.path.exists(path):
                messagebox.showerror("File not found", f"project.js not found:\n{path}")
                return
            ok, msg, backup = remove_many_project_media_file(path, links)
            if ok:
                self._last_backup_project = backup
                self._mark_touched_project()
                messagebox.showinfo("Deleted", msg)
                self.refresh_previews()
                if self.auto_commit_var.get():
                    self.git_push()
    ...
    # ----------------------------
    # backup cleanup before push
    # ----------------------------

    def _cleanup_backups_before_push(self):
        if not self.delete_backups_before_push_var.get():
            return

        for p in [self._last_backup_image, self._last_backup_project]:
            try:
    ...
    def _relpath_in_repo(self, repo: str, abs_path: str) -> str | None:
        try:
            abs_path = os.path.abspath(abs_path)
            repo = os.path.abspath(repo)
            rel = os.path.relpath(abs_path, repo)
            # if file is outside repo, rel will start with ..
            if rel.startswith(".."):
                return None
            return rel
        except Exception:
    ...
    def git_push(self):
        ...
        if self.push_image_var.get():
            img_abs = self.image_links_path.get().strip()
            rel = self._relpath_in_repo(repo, img_abs)
            if not rel:
                messagebox.showerror("Git Push", "imageLinks.js is not inside this git repo folder.")
                return
            stage_paths.append(rel)

        if self.push_project_var.get():
            prj_abs = self.project_js_path.get().strip()
            rel = self._relpath_in_repo(repo, prj_abs)
            if not rel:
                messagebox.showerror("Git Push", "project.js is not inside this git repo folder.")
                return
            stage_paths.append(rel)

        if not stage_paths:
            messagebox.showwarning("Git Push", "Select at least one file to push (imageLinks.js / project.js).")
            return

        # optional: delete backups before staging so they never get committed
        self._cleanup_backups_before_push()

        # stage ONLY the selected files
        add = self._run_git(["add"] + stage_paths, repo)
        if add.returncode != 0:
            messagebox.showerror("Git add failed", (add.stderr or add.stdout).strip())
            return

        # commit if enabled and there is something staged
        if self.auto_commit_var.get():
            staged = self._run_git(["diff", "--cached", "--name-only"], repo)
            if staged.returncode != 0:
                messagebox.showerror("Git error", (staged.stderr or staged.stdout).strip())
                return
    ...
    def github_trigger_actions(self):
        ...
        try:
            ...
            if resp.status_code in (204, 201):
                ...
                messagebox.showerror("GitHub Actions", f"Failed ({resp.status_code}).\n\n{resp.text}")
    ...
    # ----------------------------
    # Filtering + preview list refresh
    # ----------------------------
    ...
    def refresh_previews(self):
        ...
        if os.path.exists(img_path):
            try:
                ...
                self._image_all = parse_image_links(js)
            except Exception as ex:
                self._image_all = [f"[Error reading file] {ex}"]
        else:
            self._image_all = ["[File not found]"]
        ...
        if os.path.exists(prj_path):
            try:
                ...
                items = parse_project_media_items(js)
                self._project_all = [f'{it["mediaType"]} | type={it["type"]} | {it["link"]}' for it in items]
            except Exception as ex:
                self._project_all = [f"[Error reading file] {ex}"]
        else:
            self._project_all = ["[File not found]"]

        self.apply_filters()

    def apply_filters(self):
        img_q = self.image_filter_var.get().strip().lower()
        prj_q = self.project_filter_var.get().strip().lower()

        self.image_list.delete(0, tk.END)
        for l in self._image_all:
            if (not img_q) or (img_q in l.lower()):
                self.image_list.insert(tk.END, l)

        self.project_list.delete(0, tk.END)
        for row in self._project_all:
            if (not prj_q) or (prj_q in row.lower()):
                self.project_list.insert(tk.END, row)
    ...
    def add_all(self):
        ...
        if mode == "featured":
            ...
            if not os.path.exists(path):
                messagebox.showerror("File not found", f"imageLinks.js not found:\n{path}")
                return
            ok, msg, backup = add_many_image_links_file(path, links, allow_dupes)
            if ok:
                self._last_backup_image = backup
                self._mark_touched_image()
            ...
            ok, msg, backup = add_many_project_media_file(path, links, typ, media_type, allow_dupes)
            if ok:
                self._last_backup_project = backup
                self._mark_touched_project()
                messagebox.showinfo("Done", msg)
    ...
    def delete_selected(self):
        mode = self.mode.get()
        if not messagebox.askyesno("Delete", "Delete the selected link(s)?"):
            return

        if mode == "featured":
            sel = self.image_list.curselection()
            if not sel:
                messagebox.showwarning("Delete", "Select one or more links in imageLinks.js list.")
                return
            links = [self.image_list.get(i).strip() for i in sel]
            path = self.image_links_path.get().strip()
            if not os.path.exists(path):
                messagebox.showerror("File not found", f"imageLinks.js not found:\n{path}")
                return
            ...
            sel = self.project_list.curselection()
            if not sel:
                messagebox.showwarning("Delete", "Select one or more items in project.js list.")
                return
            links = []
            for i in sel:
                row = self.project_list.get(i)
    ...
    def delete_from_textbox(self):
        links = self.get_links_from_textbox()
        if not links:
            messagebox.showwarning("Delete", "Paste links (one per line) in the textbox first.")
            return

        if not messagebox.askyesno("Delete", "Delete ALL links listed in the textbox from the active mode file?"):
            return
    ...
    def undo_last_delete(self):
        ...
        if mode == "featured":
            target = self.image_links_path.get().strip()
            backup = self._last_backup_image
            if not backup or not os.path.exists(backup):
                messagebox.showinfo("Undo", "No backup available for imageLinks.js yet.")
                return
        else:
            target = self.project_js_path.get().strip()
            backup = self._last_backup_project
            if not backup or not os.path.exists(backup):
                messagebox.showinfo("Undo", "No backup available for project.js yet.")
                return

        if not messagebox.askyesno("Undo Delete", f"Restore from backup?\n\n{os.path.basename(backup)}"):
            return
        ...
        try:
            ...
            messagebox.showinfo("Undo", "✅ Restored from backup.")
            self.refresh_previews()
            if self.auto_commit_var.get():
                self.git_push()
        except Exception as ex:
            messagebox.showerror("Undo failed", str(ex))
