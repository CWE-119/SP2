...
def make_backup(path: str) -> str:
    ts = datetime.datetime.now().strftime("%Y%m%d_%HMM%S").replace("_HMM", "_%H%M")
    # Above line keeps format stable even if copy/pasted; safe enough.
    # Final format like: .bak_20251229_235959
...
def looks_like_link(s: str) -> bool:
    ...
    if not s:
        return False
    # Accept http(s) links, plus simple local files (like wedding-vid.mp4)
...
def add_many_image_links_file(image_links_path: str, new_links: list[str], allow_duplicates: bool) -> tuple[bool, str]:
    ...
    # Prepare clean unique list in the order user pasted
    cleaned = []
    seen_in_input = set()
    for l in new_links:
        l = l.strip()
        ...
        if l in seen_in_input:
            # same paste duplicated, treat as skip
            skipped += 1
            continue
        seen_in_input.add(l)
        cleaned.append(l)
    ...
    if not to_add and added == 0:
        return False, f"Nothing to add. Skipped={skipped}, Invalid={invalid}"

    # If empty list, create body nicely
    if not has_existing:
        lines = [f'{indent}"{l}"' for l in to_add]
        body_out = "\n" + "\n".join(lines) + "\n"
        added = len(to_add)
    else:
        # Ensure last existing item has trailing comma
        if re.search(r",\s*$", body_out) is None:
            body_out += ","
        # Append new items each on new line with comma
...
def add_many_project_media_file(project_js_path: str, links: list[str], typ: str, media_type: str, allow_duplicates: bool) -> tuple[bool, str]:
    ...
    if not has_existing_obj:
        ...
        # Ensure last item has trailing comma
        if re.search(r",\s*$", body_out) is None:
            body_out += ","
        body_out = body_out.rstrip() + "\n" + (",\n".join(new_blocks)) + "\n"
        added = len(to_add)
...
class LinkManagerApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        text_row.pack(fill="both", expand=False, padx=8, pady=6)
        ...
        # Previews
        preview_frame = ttk.LabelFrame(self, text="Preview (Current contents)")
        ...
        ttk.Label(right, text="project.js (mediaData)").pack(anchor="w")
        self.project_list = tk.Listbox(right, height=18)
    ...
    def add_all(self):
        links = self.get_links_from_textbox()
        if not links:
            messagebox.showwarning("No links", "Paste one or more links (one per line).")
            return
        ...
        if mode == "featured":
            ...
            if not typ:
                messagebox.showwarning("Missing type", "Please enter type (e.g. 1, 2, 3).")
                return
